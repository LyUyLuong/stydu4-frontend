...
const Login = () => {
  ...
  const returnUrl = location.state?.returnUrl || '/tests';
  ...
  useEffect(() => {
    ...
    if (token) {
      // OAuth2 login successful - save both tokens
      ...
      // Load user info
      setToken(token).then((result) => {
        if (result.success) {
          toast.success('Login successful!');
          navigate(returnUrl);
        } else {
          toast.error(result.error || 'Failed to load user info');
        }
      });
    } else if (error) {
      // OAuth2 login failed
      const message = searchParams.get('message') || 'OAuth2 login failed';
      toast.error(message);
